...
for key, default in [
    ...
    ("history_stack",    None),
    ("slide_toggles",    None),
    ("mode_saisie",      "texte"),  # "texte" | "questionnaire"
    ("quest_avec_texte", False),
    ("auto_generer",     False),
]:
...
if st.session_state.resume is None:
    ...
    # ── Mode questionnaire ────────────────────────────────────
        ...
        # Toggle "Prendre en compte le texte libre"
        with_texte = st.toggle(
            "✍️ Prendre en compte le texte libre",
            value=st.session_state.get("quest_avec_texte", False),
            key="tgl_quest_avec_texte",
            help="Activez pour ajouter des informations libres en complément du questionnaire.",
        )
        st.session_state.quest_avec_texte = with_texte
        if with_texte:
            texte_libre_hors_form = st.text_area(
                "✍️ Texte libre complémentaire",
                placeholder="Ajoutez ici tout contexte supplémentaire, contraintes, remarques…",
                height=120,
                key="texte_libre_compl",
            )
        else:
            texte_libre_hors_form = ""
        ...
        if submitted:
            ...
            # Récupérer le texte libre saisi hors formulaire
            texte_compl = st.session_state.get("texte_libre_compl", "").strip()
            if texte_compl:
            ...
            st.session_state.messages.append({
                ...
                "content": (
                    ...
                    + (", complété par du texte libre" if texte_compl else "") + ". "
    ...
    with chat_col:
        ...
        with c2:
            if st.button("🔄 Recommencer", use_container_width=True):
                ...
                st.session_state.mode_saisie = "texte"
                st.session_state.quest_avec_texte = False
